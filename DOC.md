...
A per-tenant API rate limiter built on Spring Boot, Redis and Spring Cloud Gateway.
...
| `api-gateway` | Spring Cloud Gateway, the filter, JWT validation, tenant cache, admin endpoints. |
| `service-a`, `service-b` | Trivial downstreams with a `/chaos` toggle so the circuit breaker can be demonstrated without killing containers. |
| `infra` | Dockerfile, Prometheus scrape config, provisioned Grafana dashboard. |

### Request path in code

1. `RateLimiterGatewayFilterFactory` — the filter, applied per route in `application.yml`
2. `JwtTenantResolver` — verifies the HMAC signature, pulls `tenantId` and `tier` claims
3. `CachingTenantConfigService` — Redis cache in front of Postgres
4. `RateLimiterFactory` — strategy dispatch on the tenant's configured algorithm
5. `TokenBucketLimiter` / `SlidingWindowLimiter` — each wraps one Lua script
...
| Downstream failing | Circuit opens after 5 failures in a 10-call window, `/fallback/{service}` returns a structured 503 | Stops a broken downstream from consuming gateway connections on timeouts it will never satisfy. |
...
# hit /api/a/ping a few times — breaker opens, /fallback/service-a serves 503
...
**"Why Lua instead of `INCR`?"**
`INCR` is atomic and works fine for a fixed window. It cannot express a token bucket,
because refill depends on elapsed time since the last request — that requires reading
state, computing, and writing back. Lua is what makes that sequence atomic. A fixed-window
counter also has the boundary problem: 60 requests at 11:59:59 and 60 more at 12:00:00 is
120 requests in one second, all within quota.

**"Token bucket or sliding window?"**
Token bucket by default. Measured, the sliding window costs roughly 1,000× the memory — around 950 MB
versus 0.9 MB at 10,000 tenants — for smoothing that most API clients don't need, since
bursty traffic is normal rather than abusive. Sliding window is available per tenant where
the downstream genuinely can't absorb bursts.

**"Fail open or fail closed?"**
Open, by default, because the limiter protects capacity rather than correctness, and a
Redis blip becoming a customer 503 is a worse outcome than brief over-admission. It is a
config flag because a metered billing API should choose the opposite — unbilled requests
cost real money there.

**"How do you know it's actually correct?"**
The concurrency test, and the naive implementation I benchmarked against it: same
workload, same Redis, 640 admitted against a quota of 100 versus exactly 100 for the
atomic version, reproducible across runs.

**"What breaks first at scale?"**
Redis, on two axes. Connection count and CPU on the hot path — addressed by Cluster and
eventually by a two-layer local/global design. Memory, if the sliding window is used
widely — the ~1,000× figure is why the token bucket is the default.
